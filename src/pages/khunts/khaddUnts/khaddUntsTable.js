...
import  AddUntsPopup from "./khaddUntsPopup"
...
function AddKhUntsTable({customers,SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
        setLoading(true)
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES27).createPrison( editobj.EmployeeFormID, editobj.Employee27ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات    الخالات ' ,  "تعديل  بيانات الخالات  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES27).delete( deletedObj.EmployeeFormID , deletedObj.Employee27ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   الخالات ' ,  "حذف صف من  بيانات  الخالات  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
  ...
  setOpenPopup(true)  
   ...
   {remove !== true ? (
                ...
                {customers.listData.map((cust) => (
                    <TableRow key={cust.Employee27ID}>
                      <TableCell>{cust.Name27}</TableCell>
                      <TableCell>{(cust.BDay27).slice(0, 10)}</TableCell>
                      <TableCell>{cust.Age27}</TableCell>
                      <TableCell> {cust.Nationality27} </TableCell>
                      <TableCell> {cust.Job} </TableCell>
                      <TableCell> {cust.Address} </TableCell>
   {remove !== true ? (
                      <TableCell>
                        <IconButton
                         onClick={() => getOneCustomer( cust,cust.Employee27ID)} 
                         color="primary"
