...
import AddKhUntsTable from "./khaddUntsTable"
...
var today = new Date();
var birthDate = new Date(values.BDay27);
  ...
  // const hist =[]
   function handleSubmit(e) {
    ...
    if (validateForm()) {
      ...
      createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES27).createPrison( customers.EmployeeFormID, "0" ,values)
      .then(res => {
          setLoading(false)
          notify()
          SetCustomers(res.data.data)
      ...
      .then(res => {
        handlehistory( action.ADD , 'بيانات   الخالات ' ,  "  اضافه  بيانات الخالات    ")
      })
   ...
   function handleNext(e){
    e.preventDefault()
    history.push(`/KhUntsHusband/${id}/${type}`)
   }
            ...
            />
             ...
             <Grid item  xs={12} sm={6} md={4} lg={4}>
                <Input
                     name="Nationality27"
                     label=" الجنسيه"
                     value={values.Nationality27}
                     error={errors.Nationality27}
                     onChange={handleInputChange}
                     type= "text"
                 />
            </Grid>
   ...
   <div style={{display:"none"}}>
    <ComponentToPrint
              ...
              Table={AddKhUntsTable}
              customers={customers}
              SetCustomers = {SetCustomers}
              title={'  بيانات   الخالات '}
              ref={componentRef} 
              remove={remove}
              code={id}
              />
