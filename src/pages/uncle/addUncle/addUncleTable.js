...
import  AddUnclePopup from "./addUnclePopup"
...
function AddUncleTable({customers,SetCustomers ,remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
        console.log(editobj);
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES18).createPrison( editobj.EmployeeFormID, editobj.Employee18ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات    الاعمام ' ,  "تعديل  بيانات الاعمام  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES18).delete( deletedObj.EmployeeFormID , deletedObj.Employee18ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   الاعمام ' ,  "حذف صف من  بيانات  الاعمام  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
        ...
        {customers  ?  (
          ...
          {remove !== true ? (
                ...
                {customers.listData.map((cust) => (
                    <TableRow key={cust.Employee18ID}>
                      <TableCell>{cust.Name18}</TableCell>
                      <TableCell>{(cust.BDay18).slice(0, 10)}</TableCell>
                      <TableCell>{cust.Age18}</TableCell>
                      <TableCell> {cust.Nationality18} </TableCell>
          ...
          {remove !== true ? (
                      ...
                      <TableCell>
                        <IconButton
                         onClick={() => getOneCustomer( cust,cust.Employee18ID)} 
                         color="primary"
