...
function  NationalityTable({customers ,setCrime ,SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
          console.log(editobj);
          createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES7).createPrison( editobj.EmployeeFormID, editobj.EmployeeNationalityID ,editobj)
          .then(res => {
            handleClose(true)
            setLoading(false)
          ...
          .then(res => {
            handlehistory( action.EDIT , 'بيانات   الجنسيه  ' ,  "تعديل   بيانات   الجنسيه   ")
          })
  ...
  const ConfirmDeleteHandler = async () => {
    setLoading(true)
      createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES7).delete( deletedObj.EmployeeFormID , deletedObj.EmployeeNationalityID )
      .then(res => {
       console.log(res)
       SetCustomers(res.data.data)
      ...
      .then(res => {
        handlehistory( action.DELETE , 'بيانات   الجنسيه  ' ,  "حذف صف من  بيانات الجنسيه     ")
      })
        .catch(function (response) {
          handleClose();
          setLoading(false)
        ...
        {customers ?  (
            <>
                ...
                <TableRow>
                    ...
                    <TableCell>    انت او احد اقاربك مع ذكر درجه القرابه </TableCell>
                ...
                {customers.listData.map((cust) => (
                    <TableRow 
                    key={cust.EmployeeNationalityID}
                     id={cust.EmployeeNationalityID}
                    >
                      <TableCell>{cust.Nat_Name}</TableCell>
                      <TableCell>{cust.Nat_Degree}</TableCell>

                      <TableCell> {cust.Nat_Type} </TableCell>
                      {remove !== true ? (
