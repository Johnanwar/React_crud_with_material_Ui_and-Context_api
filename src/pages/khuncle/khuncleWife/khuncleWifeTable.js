...
import  UncleWifePopup from "./khuncleWifePopup"
...
function UncleKhWifeTable({customers , SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
        setLoading(true)
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES22).createPrison( editobj.EmployeeFormID, editobj.Employee22ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات    زوجات الاخوال ' ,  "تعديل  بيانات زوجات الاخوال  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES22).delete( deletedObj.EmployeeFormID , deletedObj.Employee22ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   زوجات الاخوال ' ,  "حذف صف من  بيانات  زوجات الاخوال  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
 ...
 {customers ?  (
      <>
          ...
          {customers.listData.map((cust) => (
              <TableRow key={cust.Employee22ID}>
                <TableCell>{cust.Name22}</TableCell>
                <TableCell>{(cust.BDay22).slice(0, 10)}</TableCell>
                <TableCell>{cust.Age22}</TableCell>
                <TableCell> {cust.Nationality22} </TableCell>
                <TableCell> {cust.Job} </TableCell>
                <TableCell> {cust.Address} </TableCell>
              {remove !== true ? (
                <TableCell>
                  <IconButton
                   onClick={() => getOneCustomer( cust,cust.Employee22ID)} 
                   color="primary"
...
export default UncleKhWifeTable
