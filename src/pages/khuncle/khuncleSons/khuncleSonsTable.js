...
import  UncleSonsPopup from "./khuncleSonsPopup"
import {notifyUpdate,notifyDelete , notifyErr} from "../../../components/alerts/alert"
import Popup from "../../../shared/Popup";
import {Alert} from "../../../components/alerts/alert";
import {HistoryContext} from '../../../contexts/historyContext'

function UncleKhSonsTable({customers , SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
        setLoading(true)
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES23).createPrison( editobj.EmployeeFormID, editobj.Employee23ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات    أولاد الاخوال ' ,  "تعديل  بيانات أولاد الاخوال  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES23).delete( deletedObj.EmployeeFormID , deletedObj.Employee23ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   أولاد الاخوال ' ,  "حذف صف من  بيانات  أولاد الاخوال  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
 ...
 {customers  ?  (
      <>
          ...
          {customers.listData.map((cust) => (
              <TableRow key={cust.Employee23ID}>
                <TableCell>{cust.Name23}</TableCell>
                <TableCell>{(cust.BDay23).slice(0, 10)}</TableCell>
                <TableCell>{cust.Age23}</TableCell>
                <TableCell> {cust.Nationality23} </TableCell>
                <TableCell> {cust.Job} </TableCell>
                <TableCell> {cust.Address} </TableCell>
                {remove !== true ? (
                <TableCell>
                  <IconButton
                   onClick={() => getOneCustomer( cust,cust.Employee23ID)} 
                   color="primary"
...
export default UncleKhSonsTable
