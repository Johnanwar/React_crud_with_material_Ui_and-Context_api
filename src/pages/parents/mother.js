      ...
      createAPIEndpoint(ENDPIONTS.EMPLOYEES12).fetchByIdAndYear(match.params.id,match.params.type)
          .then(res => {
            if(res.data.data!= null){
              ...
              const fatherobj =res.data.data.listData.find(x => x.ParentType == "الام")
...
setValues(getFreshModelObject);
  ...
  function handleSubmit(e) {
    ...
    if (validateForm()) {
      ...
      .then(res => {
        handlehistory( action.ADD , 'بيانات   الام ' ,  "  اضافه  بيانات الام    ")
      })
      .then(res => {
        notify();
        setLoading(false)
        history.push(`/AddBrothers/${match.params.id}/${match.params.type}`)
