...
import BrothersWifesPopup from "./BrothersWifesPopup"
...
function BrothersWifesTable({customers ,SetCustomers ,remove}) {
  ...
  const handleInputChange = e => {
    ...
    function handleSubmit(e) {
      ...
      setLoading(true)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات     اشقاء وشقيقات الزوجه ' ,  "تعديل  بيانات  اشقاء وشقيقات الزوجه  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES16).delete( deletedObj.EmployeeFormID , deletedObj.EmployeePart_BroID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات    اشقاء وشقيقات الزوجه ' ,  "حذف صف من  بيانات   اشقاء وشقيقات الزوجه  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
    ...
    <>
        ...
        {customers.listData.map((cust) => (
            ...
            >
              ...
              <TableCell> {cust.Part_BroAge} </TableCell>
...
export default BrothersWifesTable
