...
import AddUntsTable from "./addUntsTable"
...
var today = new Date();
var birthDate = new Date(values.BDay24);
...
setValues(getFreshModelObject);
  ...
  function handleSubmit(e) {
    ...
    if (validateForm()) {
      ...
      createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES24).createPrison( customers.EmployeeFormID, "0" ,values)
      .then(res => {
          setLoading(false)
          notify()
          resetFormControls();
      ...
      .then(res => {
        handlehistory( action.ADD , 'بيانات   العمات ' ,  "  اضافه  بيانات العمات    ")
      })
   ...
   function handleNext(e){
    e.preventDefault()
    history.push(`/UntsHusband/${id}/${type}`)
   }
            ...
            />
             ...
             <Grid item  xs={12} sm={6} md={4} lg={4}>
                <Input
                     name="Nationality24"
                     label=" الجنسيه"
                     value={values.Nationality24}
                     error={errors.Nationality24}
                     onChange={handleInputChange}
                     type= "text"
                 />
            </Grid>
