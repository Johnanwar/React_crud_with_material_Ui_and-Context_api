...
import  UntsHusbandPopup from "./untsHusbandPopup"
...
function UntsHusbandTable({customers , SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    UserID:UserID,
      ...
      function handleSubmit(e) {
        e.preventDefault()
        setLoading(true)
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES25).createPrison( editobj.EmployeeFormID, editobj.Employee25ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات     ازواج العمات ' ,  "تعديل  بيانات  ازواج العمات  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES25).delete( deletedObj.EmployeeFormID , deletedObj.Employee25ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   ازواج العمات ' ,  "حذف صف من  بيانات  ازواج العمات  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
 ...
 {customers  ?  (
      <>
          ...
          {customers.listData.map((cust) => (
              <TableRow key={cust.Employee25ID}>
                <TableCell>{cust.Name25}</TableCell>
                <TableCell>{(cust.BDay25).slice(0, 10)}</TableCell>
                <TableCell>{cust.Age25}</TableCell>
                <TableCell> {cust.Nationality25} </TableCell>
                <TableCell> {cust.Job} </TableCell>
                <TableCell> {cust.Address} </TableCell>
{remove !== true ? (
                <TableCell>
                  <IconButton
                   onClick={() => getOneCustomer( cust,cust.Employee25ID)} 
                   color="primary"
...
export default UntsHusbandTable
