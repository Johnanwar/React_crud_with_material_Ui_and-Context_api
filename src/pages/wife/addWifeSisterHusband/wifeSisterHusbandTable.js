...
import WifeSisterHusbandPopup from "./wifeSisterHusbandPopup"
...
function WifeSisterHusbandTable({customers ,SetCustomers , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const handleInputChange = e => {
    ...
    function handleSubmit(e) {
      e.preventDefault()
      setLoading(true)
        createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES17).createPrison( editobj.EmployeeFormID, editobj.Employee17ID ,editobj)
        .then(res => {
          handleClose(true)
          setLoading(false)
        ...
        .then(res => {
          handlehistory( action.EDIT , 'بيانات    أزواج شقيقات الزوجه او الخطيبه ' ,  "تعديل  بيانات أزواج شقيقات الزوجه او الخطيبه  ")
        })
...
const ConfirmDeleteHandler = async () => {
  setLoading(true)
    createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES17).delete( deletedObj.EmployeeFormID , deletedObj.Employee17ID )
    .then(res => {
     console.log(res)
     SetCustomers(res.data.data)
     setOpenPopup(false)
    ...
    .then(res => {
      handlehistory( action.DELETE , 'بيانات   أزواج شقيقات الزوجه او الخطيبه ' ,  "حذف صف من  بيانات  أزواج شقيقات الزوجه او الخطيبه  ")
    })
      .catch(function (response) {
        handleClose();
        setLoading(false)
      ...
      {customers  ?  (
          <>
              ...
              {customers.listData.map((cust) => (
                  ...
                  >
                    <TableCell>{cust.Name17}</TableCell>
                    <TableCell>{cust.BDay17.slice(0, 10 )}</TableCell>
                    <TableCell> {cust.Age17} </TableCell>
                    <TableCell> {cust.Nationality17} </TableCell>
...
export default WifeSisterHusbandTable
