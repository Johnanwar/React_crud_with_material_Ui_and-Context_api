...
import RightNavBottom from "../../components/nav/rightNavBottom"
...
function WeaponLicence({match}) {
  const { handlehistory ,action ,UserID} = useContext(HistoryContext)
    ...
    const [DocType, setDocType] = useState(5); 
 ...
 function handleSubmit(e) {
        ...
        console.log(DocImage);
          ...
          .then(res => {
            handlehistory( action.ADD , 'تحميل صورة   رخصة السلاح       ' ,  "  تحميل صورة     رخصة السلاح         ")
            history.push(`/`)
            notify()
          })
