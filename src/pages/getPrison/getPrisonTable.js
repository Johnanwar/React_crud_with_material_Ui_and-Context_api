...
import GetPrisonPopup from "./getPrisonPopup"
...
function GetPrisonTable({customers,SetCustomers ,setCrime , remove}) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleInputChange = e => {
      ...
      function handleSubmit(e) {
        e.preventDefault()
        setLoading(true)
          createAPIEndpoint(ENDPIONTS.POSTEMPLOYEES6).createPrison( editobj.EmployeeFormID, editobj.EmployeeRelativeID ,editobj)
          .then(res => {
            handleClose(true)
            setLoading(false)
          ...
          .then(res => {
            handlehistory( action.EDIT , 'بيانات النشاط السياسي ' ,  "تعديل   بيانات النشاط السياسي  ")
          })
  ...
  const ConfirmDeleteHandler = async () => {
    setLoading(true)
      createAPIEndpoint(ENDPIONTS.DELETEEMPLOYEES6).delete( deletedObj.EmployeeFormID , deletedObj.EmployeeRelativeID )
      .then(res => {
       console.log(res)
       SetCustomers(res.data.data)
      ...
      .then(res => {
        handlehistory( action.DELETE , 'بيانات النشاط السياسي ' ,  "حذف صف من  بيانات النشاط السياسي  ")
      })
        .catch(function (response) {
          handleClose();
          setLoading(false)
        ...
        {customers ?  (
            <>
                ...
                <TableRow>
                    ...
                    <TableCell>   سبب الاتهام </TableCell>
                    {remove !== true ? (
                ...
                {customers.listData.map((cust) => (
                    <TableRow key={cust.EmployeeRelativeID}
                    id={cust.EmployeeRelativeID}>
                      <TableCell>{cust.Rel_Name}</TableCell>
                      <TableCell>{cust.Rel_Job}</TableCell>
                      <TableCell> {cust.Rel_Degree} </TableCell>
                      <TableCell> {cust.Rel_Add} </TableCell>
                      <TableCell> {cust.Rel_Reason} </TableCell>
